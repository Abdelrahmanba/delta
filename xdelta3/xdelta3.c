...
int xd3_config_stream(xd3_stream *stream, xd3_config *config) {
    ...
    switch (config->smatch_cfg) {
        ...
        IF_BUILD_DEFAULT(
            case XD3_SMATCH_DEFAULT : *smatcher = __smatcher_default; break;)
        IF_BUILD_SLOW(case XD3_SMATCH_SLOW : *smatcher = __smatcher_slow;
                      break;)
        IF_BUILD_FASTEST(
            case XD3_SMATCH_FASTEST : *smatcher = __smatcher_fastest; break;)
        IF_BUILD_FASTER(case XD3_SMATCH_FASTER : *smatcher = __smatcher_faster;
                        break;)
        IF_BUILD_FAST(case XD3_SMATCH_FAST : *smatcher = __smatcher_fast;
                      break;)
        default:
...
int xd3_encode_input(xd3_stream *stream) {
    ...
    switch (stream->enc_state) {
        ...
        enc_flush:
            ...
            IF_DEBUG2(DP(
                RINT "[WINSTART:%" Q "u] input bytes %" W "u offset %" Q "u\n",
...
static int xd3_source_match_setup(xd3_stream *stream, xoff_t srcpos) {
    ...
    if (srcpos < stream->srcwin_cksum_pos &&
        ...
        IF_DEBUG2(DP(RINT "[match_setup] rejected due to src->max_winsize "
                          "distance eof=%" Q "u srcpos=%" Q "u max_winsz=%" Q
                          "u\n",
        ...
        while (stream->match_back < stream->match_maxback) {
            ...
            if ((ret = xd3_getblk(stream, tryblk))) {
                if (ret == XD3_TOOFARBACK) {
                    IF_DEBUG2(DP(RINT "[maxback] %" Q "u TOOFARBACK: %" W
                                      "u INP %" Q "u CKSUM %" Q "u\n",
            ...
            tryrem =
                xd3_min(tryoff, stream->match_maxback - stream->match_back);
...
donefwd:
    ...
    IF_DEBUG2(DP(RINT "[extend match] input %" Q "u srcpos %" Q "u len %" W
                      "u\n",
    ...
    if (stream->match_fwd < stream->min_match) {
       ...
       values set to 0. */
        ...
        IF_DEBUG2({
            static int x = 0;
            DP(RINT "[source match:%d] length %" W "u <inp %" Q "u %" Q
                    "u>  <src %" Q "u %" Q "u> (%s)\n",
...
restartloop:
    ...
    if (xd3_iopt_last_matched(stream) > stream->input_position) {
        stream->min_match =
            xd3_max(MIN_MATCH,
                    1 + xd3_iopt_last_matched(stream) - stream->input_position);
    } else {
        stream->min_match = MIN_MATCH;
    }
...
#define HANDLELAZY(mlen)                                                    \
    ...
    for (;; inp += 1, stream->input_position += 1) {
        ...
        if (DO_SMALL) {
            ...
            if (match_length >= stream->min_match) {
                IF_DEBUG2({
                    static int x = 0;
                    DP(RINT "[target match:%d] <inp %" W "u %" W "u>  <cpy %" W
                            "u %" W
                            "u> "
                            "(-%" W "d) [ %" W "u bytes ]\n",
    ...
    updateone:
        ...
        if (DO_LARGE && (stream->input_position + LLOOK < stream->avail_in)) {
            lcksum =
                xd3_large_cksum_update(&stream->large_hash, lcksum, inp, LLOOK);
        }
    }
